...
const MAX_COST = new BN('500000000000000') // 0.005 ether
...
const createBaseClient = opts => {
  ...
  const get = ({ path, query }) => utils.get({
    url: `${baseUrl}${path}`,
    query,
    headers,
  })

  const post = ({ path, body }) => utils.post({
    url: `${baseUrl}${path}`,
    body,
    headers,
  })
  ...
  const getProcessedBlockNumber = () => get({ path: '/blockNumber' })
  ...
  const isSynced = pMemoize(async () => {
    const behind = await getBlocksBehindEtherscan()
...
const createTransactor = opts => {
  ...
  const signAndSend = async ({
    ...
    if (typeof params.gasPrice === 'string') {
      params.gasPrice = parseInt(params.gasPrice, 10)
    }

    const tx = new EthereumTx(params)
    const cost = tx.getUpfrontCost()
    if (cost.cmp(maxCost) > 0) {
      throw new Error(`too expensive, upfront cost is ${cost.toString(16)} wei, max allowed cost is ${maxCost.toString(16)} wei`)
    }

    tx.sign(privateKey)
    const txHex = prefixHex(tx.serialize().toString('hex'))
    ...
    return {
      txId: prefixHex(tx.hash(true).toString('hex'))
    }
  }
